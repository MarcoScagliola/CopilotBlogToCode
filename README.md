...
- Bronze, Silver, and Gold each use separate storage and identities.
...
2. Run Deploy Infrastructure workflow with defaults:
   - workload: blg
   - environment: dev
...
4. Run Deploy DAB workflow using the infra run id.
5. Trigger orchestrator_job in Databricks to execute setup -> bronze -> silver -> gold -> smoke-test.

## Runtime Jobs

- setup_job: creates Unity Catalog catalogs and schemas.
- bronze_job: ingests seed events and writes Bronze table.
- silver_job: deduplicates and standardizes from Bronze to Silver.
- gold_job: aggregates Silver to Gold.
