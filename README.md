...
This repository implements the secure medallion pattern from the Azure Databricks blog post using Terraform for Azure and Databricks workspace-adjacent infrastructure, and a Databricks Asset Bundle for Lakeflow Jobs and Python entrypoints. Bronze, Silver, and Gold are isolated by storage, identity, and compute so each layer can run with tightly scoped permissions and independent operational controls.

## Architecture Overview

The generated baseline provisions one Azure Databricks workspace, one Key Vault, and three storage-backed medallion layers. Terraform owns the Azure, Entra, access connector, and Unity Catalog-adjacent setup; the Databricks Asset Bundle owns the Bronze, Silver, Gold, and orchestrator jobs.
...
- A GitHub Environment named `BLG2CODEDEV` containing `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`, `AZURE_CLIENT_ID`, and `AZURE_CLIENT_SECRET`.
- A Databricks workspace admin or equivalent principal able to validate Unity Catalog availability, workspace principals, and storage credential behavior after deployment.
...
3. Add `AZURE_CLIENT_ID` and `AZURE_CLIENT_SECRET` to the GitHub Environment `BLG2CODEDEV`.
