...
variable "sp_object_id" {
  type        = string
  description = "Deployment service principal Enterprise Application object ID. Used for RBAC assignments and Key Vault access policy. Sourced from GitHub Secrets (AZURE_SP_OBJECT_ID)."
  sensitive   = true
}
...
variable "key_vault_recover_soft_deleted" {
  type        = bool
  description = "When true, the azurerm provider will attempt to recover a soft-deleted Key Vault with the same name rather than fail."
  default     = true
}
