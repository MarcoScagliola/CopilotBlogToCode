...
variable "layer_service_principal_mode" {
  description = "How layer service principals are sourced: create per-layer identities or reuse one existing identity."
  type        = string
  default     = "create"
...
variable "existing_layer_sp_client_id" {
  description = "Client ID of an existing service principal to reuse when layer_service_principal_mode is existing."
  type        = string
  default     = ""
}
...
variable "storage_shared_key_enabled" {
  description = "Compatibility default for Storage Account provisioning with the AzureRM provider."
  type        = bool
  default     = true
}
