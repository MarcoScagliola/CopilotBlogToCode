locals {
  ...
  storage_account_names = {
    for layer, _ in local.layer_settings :
  ...
  layer_principal_client_ids = local.create_layer_principals ? {
    for layer, _ in local.layer_settings : layer => azuread_service_principal.layer[layer].client_id
  } : {
    for layer, _ in local.layer_settings : layer => var.existing_layer_sp_client_id
  }

  layer_principal_object_ids = local.create_layer_principals ? {
    for layer, _ in local.layer_settings : layer => azuread_service_principal.layer[layer].object_id
  } : {
    for layer, _ in local.layer_settings : layer => var.existing_layer_sp_object_id
  }
}
