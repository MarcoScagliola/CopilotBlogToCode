# Read the object ID of the principal executing Terraform so Key Vault access
...
resource "azurerm_key_vault" "platform" {
  ...
  # Grant the deployment principal (the identity running Terraform) access.
  # Using data.azurerm_client_config.current.object_id ensures the correct
  # running identity gets access rather than a static var that may differ.
  access_policy {
    tenant_id = var.azure_tenant_id
...
resource "azurerm_role_assignment" "layer_blob_owner" {
  ...
  # Explicit principal_type avoids the PrincipalTypeNotSupported error that
