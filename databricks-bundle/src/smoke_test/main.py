...
def assert_table_rows(spark: SparkSession, table_name: str, minimum_rows: int) -> None:
...
def main() -> None:
    ...
    assert_table_rows(spark, f"{args.bronze_catalog}.{args.bronze_schema}.raw_events", args.min_row_count)
    assert_table_rows(spark, f"{args.silver_catalog}.{args.silver_schema}.events", args.min_row_count)
    assert_table_rows(spark, f"{args.gold_catalog}.{args.gold_schema}.event_summary", args.min_row_count)
